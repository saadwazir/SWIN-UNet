...
class patch_merging(tf.keras.layers.Layer):
    ...
    def __init__(self, num_patch, embed_dim, name=""):
        ...
        self.linear_trans = Dense(
            2 * embed_dim, use_bias=False, name="0{}_linear_trans".format(name)
        )
...
class patch_expanding(tf.keras.layers.Layer):
    def __init__(
        self, num_patch, embed_dim, upsample_rate, return_vector=True, name=""
    ):
        super().__init__()
        ...
        self.linear_trans1 = Conv2D(
            ...
            kernel_size=1,
            use_bias=False,
            name="0{}_linear_trans1".format(name),
        )
        #
    ...
    def call(self, x):
        ...
        x = tf.nn.depth_to_space(
            ...
            self.upsample_rate,
            data_format="NHWC",
            name="0{}_d_to_space".format(self.prefix),
        )
