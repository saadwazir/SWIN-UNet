...
class Mlp(tf.keras.layers.Layer):
    def __init__(self, filter_num, drop=0.0, name=""):
        super().__init__()

        # MLP layers
        self.fc1 = Dense(filter_num[0], name="0{}_mlp_0".format(name))
        self.fc2 = Dense(filter_num[1], name="0{}_mlp_1".format(name))
...
class WindowAttention(tf.keras.layers.Layer):
    def __init__(
        ...
        # Layers
        self.qkv = Dense(
            dim * 3, use_bias=qkv_bias, name="0{}_attn_qkv".format(self.prefix)
        )
    ...
    def build(self, input_shape):
        ...
        self.relative_position_index = tf.Variable(
            ...
            trainable=False,
            name="0{}_attn_pos_ind".format(self.prefix),
        )
...
class SwinTransformerBlock(tf.keras.layers.Layer):
    def __init__(
        ...
        self.dim = dim  # number of input dimensions
        self.num_patch = (
            num_patch  # number of embedded patches; a tuple of  (heigh, width)
        )
        ...
        self.norm1 = LayerNormalization(
            epsilon=1e-5, name="0{}_norm1".format(self.prefix)
        )
        ...
        self.norm2 = LayerNormalization(
            epsilon=1e-5, name="0{}_norm2".format(self.prefix)
        )
    ...
    def build(self, input_shape):
        if self.shift_size > 0:
            ...
            self.attn_mask = tf.Variable(
                ...
                trainable=False,
                name="0{}_attn_mask".format(self.prefix),
            )
        else:
            self.attn_mask = None
