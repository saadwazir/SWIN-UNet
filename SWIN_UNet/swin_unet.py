...
def swin_unet_2d_conv(
    ...
    for i in range(depth_decode):
        ...
        #     )
        # )
        reshaped = tf.keras.layers.Reshape((num_patch_x, num_patch_y, -1))(X_decode[i])
        # print(reshaped)
        X = concatenate([X, reshaped], axis=-1, name="{}_concat_{}".format(name, i))
        X = keras.layers.Conv2D(
            start, (4, 4), strides=(1, 1), padding="same", activation="relu"
        )(X)
        # X = keras.layers.BatchNormalization()(X)
